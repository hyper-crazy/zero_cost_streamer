...
class _ContentTrackScreenState extends State<ContentTrackScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ContentProvider>(
      builder: (context, provider, child) {
        List<Content> displayList = (_tabIndex == 0)
            ? provider.watchingList
            : provider.completedList;
            ...
            actions: [
              if (_isSelectionMode) ...[
                IconButton(
                  ...
                  onPressed: () => setState(
                    () => _selectedIds.addAll(filteredList.map((e) => e.id)),
                  ),
                ),
                IconButton(
            ...
            children: [
              ...
              Expanded(
                child: filteredList.isEmpty
                    ? Center(
                        child: Text(
                          "No items in ${_tabIndex == 0 ? 'Watching' : 'Completed'}",
                          ...
                          return Dismissible(
                                ...
                                if (provider.isAlreadyInCompleted(content.id)) {
                                  bool? confirm = await showDialog<bool>(
                                    ...
                                    builder: (ctx) => AlertDialog(
                                      ...
                                      content: const Text(
                                        "Already in Completed. Remove from Watching?",
                                      ),
                                      actions: [
                                        TextButton(
                                          onPressed: () =>
                                  ...
                                  if (confirm == true)
                                    provider.removeFromWatching(content);
                                  return false;
                                }
                            ...
                            onDismissed: (direction) {
                              if (direction == DismissDirection.endToStart) {
                                _tabIndex == 0
                                    ? provider.removeFromWatching(content)
                                    : provider.removeFromCompleted(content);
                              }
                            },
                            ...
                            child: GestureDetector(
                              ...
                              onTap: () {
                                if (_isSelectionMode)
                                  setState(
                                    () => _selectedIds.contains(content.id)
                                        ? _selectedIds.remove(content.id)
                                        : _selectedIds.add(content.id),
                                  );
                              },
  ...
  Widget _buildModernCard(
    ...
    String subTitle = content.mediaType == 'tv'
        ? provider.getLastEpisode(content.id)
        : "${content.mediaType.toUpperCase()} • ${content.releaseYear}";
    return Container(
        ...
        children: [
          InkWell(
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => DetailsScreen(movie: content)),
            ),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(15),
              child: Image.network(
                content.fullBackdropUrl,
                width: 80,
                height: 110,
                fit: BoxFit.cover,
              ),
          ...
          if (_tabIndex == 0 && !_isSelectionMode)
            InkWell(
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => content.mediaType == 'tv'
                      ? SeasonSelectionScreen(content: content)
                      : PlayerScreen(content: content),
                ),
              ),
              child: Column(
                children: [
                  Icon(Icons.play_circle_fill, color: cs.primary, size: 30),
                  Text(
                    "Continue",
                    style: TextStyle(
                      color: cs.primary,
  ...
  Widget _buildSegmentedTabs(ColorScheme cs, bool isDark) {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: ["Watching", "Completed"].asMap().entries.map((entry) {
